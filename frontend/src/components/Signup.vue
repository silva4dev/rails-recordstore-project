  ...
  <div class="max-w-sm m-auto my-8">
    <div class="border p-10 border-grey-light shadow rounded">
      ...
      <form @submit.prevent="signup">
        <div class="text-red" v-if="errors" v-for="(error, index) in errors" :key="index">
          {{ error }}
        </div>
        <div class="mb-6 mt-5">
          <label for="email" class="label">E-mail Address</label>
          <input type="email" v-model="email" class="input" id="email" placeholder="email@mail.com">
        </div>
...
export default {
  ...
  methods: {
    ...
    signupSuccessful (response) {
      ...
      this.errors = []
      this.$bus.$emit('logged', localStorage.signedIn)
      this.$router.replace('/records')
    },
    signupFailed (errors) {
      this.errors = (errors.response && errors.response.data && errors.response.data.error) || ['Something went wrong']
